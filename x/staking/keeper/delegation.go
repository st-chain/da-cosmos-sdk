...
func (k Keeper) UnbondImmediately(
	...
	valDelegateAddr, err := k.validatorAddressCodec.StringToBytes(delegation.GetValidatorAddr())
	if err != nil {
		return amount, err
	}
	...
	if delegation.Shares.IsZero() {
		...
		err = k.Hooks().AfterDelegationModified(ctx, delegatorAddress, valDelegateAddr)
	}
	...
	if validator.DelegatorShares.IsZero() && validator.IsUnbonded() {
		...
		valbz, errP := k.ValidatorAddressCodec().StringToBytes(validator.GetOperator())
		if errP != nil {
			return amount, errP
		}
		if err = k.RemoveValidator(ctx, valbz); err != nil {
